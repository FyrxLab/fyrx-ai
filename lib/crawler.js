...
// Site navigation/login banners ("You signed in with another tab...") are not
// documentation; most doc sites wrap the real content in <main> or <article>.
function mainContent(html) {
    const body = html.search(/<[^>]+class="[^"]*markdown-body/i); // GitHub wikis/READMEs and many markdown doc sites
    if (body >= 0) html = html.slice(body);
...
async function fetchPages(pageUrls, seedFetch) {
    ...
    for (const url of pageUrls) {
        ...
        try {
            ...
            const text = (looksLikeHtml ? stripHtml(mainContent(raw)) : raw).slice(0, MAX_TOTAL_BYTES - totalBytes);

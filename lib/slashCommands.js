...
const { SlashCommandBuilder, ContextMenuCommandBuilder, ApplicationCommandType, PermissionFlagsBits, ChannelType, MessageFlags } = require('discord.js');
const { PROVIDER_NAMES } = require('./providers');
const config = require('./config');
const { crawlDocs, refreshRoutes, routesOf } = require('./crawler');
...
const commandData = new SlashCommandBuilder()
    ...
    .addSubcommandGroup(g => g.setName('wiki').setDescription('Documentation sources')
        .addSubcommand(s => s.setName('add').setDescription('Crawl a doc site (or read a local folder) and add it as a source')
            .addStringOption(o => o.setName('name').setDescription('Short name, e.g. "solver"').setRequired(true))
            .addStringOption(o => o.setName('url').setDescription('Doc site URL, or a folder on the bot host with .md/.txt files').setRequired(true))
            .addStringOption(o => o.setName('description').setDescription('Short description (shown in /fyrxai wiki list)')))
        .addSubcommand(s => s.setName('refresh').setDescription('Re-download the pages already known for a wiki (no AI)')
            .addStringOption(o => o.setName('name').setDescription('Wiki name').setRequired(true))
            .addBooleanOption(o => o.setName('rediscover').setDescription('Also look for new pages (full crawl from the URL)')))
...
const HELP_TEXT = [
    ...
    '`/fyrxai wiki add` — escanea un sitio de docs (llms.txt/sitemap/enlaces) o una carpeta local de .md/.txt',
    '`/fyrxai wiki refresh [rediscover]` — vuelve a descargar las páginas ya conocidas (sin IA); `rediscover` busca páginas nuevas',
    'Clic derecho en un mensaje → Apps → **FyrxAI: soporte / charla / fuera de tema** — enseña al clasificador local',
...
// Right-click a message -> Apps -> label it. Stored per guild, used by lib/intent.js.
const TRAIN_LABELS = { 'FyrxAI: soporte': 'support', 'FyrxAI: charla': 'chat', 'FyrxAI: fuera de tema': 'task' };
const trainCommands = Object.keys(TRAIN_LABELS).map(name => new ContextMenuCommandBuilder()
    .setName(name).setType(ApplicationCommandType.Message)
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).toJSON());

// Pages -> docsText -> local embedding index. No AI involved.
async function storeAndIndex(guildId, name, pages) {
    if (!pages.length) return { pageCount: 0, passageCount: 0 }; // keep the old docs if every fetch failed
    const docsText = pages.map(p => `--- ${p.url} ---\n${p.text}`).join('\n\n').slice(0, 400000);
    config.setWikiDocs(guildId, name, { docsText, pageCount: pages.length });
    const passageCount = await docsIndex.buildIndex(guildId, name.toLowerCase(), docsText);
    return { pageCount: pages.length, passageCount: passageCount || 0 };
}

async function handleTrain(interaction) {
    if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageGuild)) {
        await interaction.reply({ content: 'Necesitas *Gestionar servidor*.', flags: MessageFlags.Ephemeral });
        return;
    }
    const label = TRAIN_LABELS[interaction.commandName];
    const text = config.addTrainingExample(interaction.guild.id, interaction.targetMessage.content || '', label);
    logger.log(`[FyrxAI/TRAIN] user=${interaction.user.tag} label=${label} "${text.slice(0, 80)}"`);
    const total = config.getGuildConfig(interaction.guild.id).trainingExamples.length;
    await interaction.reply({ content: text ? `Aprendido como **${label}** (${total} ejemplos en este servidor).` : 'Ese mensaje no tiene texto.', flags: MessageFlags.Ephemeral });
}

async function handleInteraction(interaction) {
    if (interaction.isMessageContextMenuCommand?.() && TRAIN_LABELS[interaction.commandName] && interaction.guild) {
    ...
    if (sub === 'status') {
        ...
        const lines = [
            ...
            `Clasificador local: ${cfg.trainingExamples.length} ejemplo(s) entrenado(s) en este servidor`,
    ...
    if (group === 'wiki') {
        if (sub === 'list') {
            ...
            await ephemeralReply(names.length
                ? names.map(n => `**${n}** — ${cfg.wikis[n].pageCount || 0} página(s), ${docsIndex.loadIndex(guildId, n)?.length || 0} pasajes indexados, actualizada ${cfg.wikis[n].crawledAt?.slice(0, 16).replace('T', ' ') || 'nunca'} — ${cfg.wikis[n].url}`).join('\n')
        ...
        if (sub === 'remove') {
            ...
            docsIndex.removeIndex(guildId, name.toLowerCase());
        ...
        if (sub === 'add' || sub === 'refresh') {
            ...
            const routes = sub === 'refresh' && !interaction.options.getBoolean('rediscover') ? routesOf(existing.docsText) : [];

            await interaction.reply({ content: `${routes.length ? `Actualizando ${routes.length} página(s) conocidas de` : 'Escaneando'} **${url}**... puede tardar unos segundos.`, flags: MessageFlags.Ephemeral });
            const pages = routes.length
                ? await refreshRoutes(url, routes)
                : await crawlDocs(url).catch((err) => { logger.error('[FyrxAI] Crawl failed:', err.message); return []; });
            const { pageCount, passageCount } = await storeAndIndex(guildId, name, pages);
            await interaction.followUp({
                content: pageCount
                    ? `Wiki **${name}** ${sub === 'add' ? 'añadida' : 'actualizada'}: ${pageCount} página(s), ${passageCount} pasajes indexados localmente.`
                    : `No pude leer ninguna página de **${url}**${sub === 'refresh' ? '; se conserva la versión anterior' : ''}. Revisa debug.txt.`,
...
module.exports = { commandData, trainCommands, handleInteraction };

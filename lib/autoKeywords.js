...
function extractSlashCommands(text) {
    // URL paths (github.com/org/repo/wiki) and the crawler's "--- url ---" page headers are not commands.
    const withoutUrls = text.replace(/https?:\/\/\S+/g, ' ');
    return [...withoutUrls.matchAll(/(?<![\w:/.\-])\/[a-z][\w-]*(?:\s+[a-z][\w-]*){0,2}/gi)].map(m => m[0].trim());
}

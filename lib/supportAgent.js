...
function getDocsContext(wiki) {
    return wiki.docsText || '';
}
...
async function answerSupportQuestion(message, config, opts = {}) {
    ...
    try {
        ...
        const sources = sourceNames.map((name) => ({ name, docsContext: getDocsContext(config.wikis[name]) }));
        ...
        console.error('[FyrxAI] Error answering support question:', err.message);
        return false;
    }
}
...
async function handleMention(message, config) {
    ...
    if (message.reference) {
        try {
            ...
            console.error('[FyrxAI] Failed to fetch referenced message, falling back to mention text:', err.message);
        }
    }
...
async function handleSupportMessage(message) {
    ...
    if (message.fyrxAssistedHandled) return false; // already answered by fyrx-assisted (canned reply)
    ...
    if (isMentioned) return handleMention(message, config);
    ...
    if (!isInSupportChannel) return false;
    ...
    const sourceNames = detectSources(message.content, config.wikis);
    if (sourceNames.length > 0) {
        if (!looksLikeSupportQuestion(message.content, config.wikis)) {
    ...
    const looksLikeQuestionStrict = (QUESTION_HINT.test(message.content) || PROBLEM_HINT.test(message.content))
        && !GREETING_HINT.test(message.content);
    ...
    const isPureCompliment = POSITIVE_SENTIMENT_HINT.test(message.content)
        && !PROBLEM_HINT.test(message.content) && !QUESTION_HINT.test(message.content);
    const semanticGuess = isPureCompliment ? null : await classifySource(message.guild.id, config.wikis, message.content);
    if (semanticGuess) {
...
module.exports = { handleSupportMessage, looksLikeSupportQuestion, detectSources, detectMessageLanguage };

...
// answers expensive; send only the chunks that share the most words with the question.
...
function getDocsContext(wiki, question) {
    ...
    for (const page of docs.split(/\n\n(?=--- https?:\/\/\S+ ---\n)/)) {
        ...
        if (!m) continue;
        for (let i = 0; i < m[2].length; i += CHUNK_CHARS) {
            const t = m[2].slice(i, i + CHUNK_CHARS);
            const lower = t.toLowerCase();
            chunks.push({ url: m[1], t, order: chunks.length, score: words.filter(w => lower.includes(w)).length });
        }
    }

...
async function answerSupportQuestion(message, config, opts = {}) {
    ...
    if (currentAttention < ATTENTION_COST) {
    ...
    try {
        ...
        spendAttention(message.author.id);
...
/**
 * Passive entry point — call for every non-command message. Only acts inside
 * a guild's configured support channels.

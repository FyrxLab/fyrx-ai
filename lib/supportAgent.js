...
// Very close to the wiki ("que es foxgate?") is a question about it whatever
// the intent classifier says - unless it is clearly a task ("write me code").
const STRONG_RELEVANCE = 0.45;
...
Reply with exactly ${NO_ANSWER} and nothing else ONLY when the message is clearly casual conversation between people, or clearly asks for something unrelated to ${topics.join(', ')} (writing or debugging code, general knowledge, homework, creative tasks, other products). When in doubt, answer.
...
async function handleSupportMessage(message) {
    ...
    if (!isMentioned) {
        const strongMatch = search.best >= STRONG_RELEVANCE && !(intent.label === 'task' && intent.confidence >= PASSIVE_MIN_CONFIDENCE);
        if (!strongMatch && (intent.label !== 'support' || intent.confidence < PASSIVE_MIN_CONFIDENCE)) {
